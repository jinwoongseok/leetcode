    ...
    def isMatch(self, s: str, p: str) -> bool:
        ...
        for i in range(1, plen + 1):
            for j in range(1, slen + 1):
                pattern_char = p[i-1]
                str_char = s[j-1]
                if pattern_char == str_char or pattern_char == '.':
                    dp[i][j] = dp[i-1][j-1]
                elif pattern_char == '*':
                    prev_pattern_char = p[i-2]
                    if prev_pattern_char != '.' and prev_pattern_char != str_char:
                        dp[i][j] = dp[i-2][j]
                    else:
                        dp[i][j] = dp[i-2][j-1] or dp[i-2][j] or dp[i][j-1]
